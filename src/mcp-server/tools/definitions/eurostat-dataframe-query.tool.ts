...
export const eurostatDataframeQuery = tool('eurostat_dataframe_query', {
  ...
  description:
    'Run a read-only SQL SELECT against tables staged on a Eurostat dataframe canvas — the way to reach observations past the 5,000-row inline cap of eurostat_query_dataset, and to aggregate, group, or join across staged tables without re-fetching from Eurostat. Call eurostat_dataframe_describe first for the table and column names. Only a single SELECT statement runs: statement chaining, non-SELECT verbs, and functions that read files or external data are rejected. Columns are flat — each dimension appears as a code column named after the dimension plus a "_label" companion, with the measure in obs_value and the status flag in obs_flag / obs_flag_label.',
