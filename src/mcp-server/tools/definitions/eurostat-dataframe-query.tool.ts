...
export const eurostatDataframeQuery = tool('eurostat_dataframe_query', {
  ...
  description:
    'Run a read-only SQL SELECT against tables staged on a Eurostat dataframe canvas — the way to reach observations past the 5,000-row inline cap of eurostat_query_dataset and past the inline preview of a eurostat_download_dataset bulk download, and to aggregate, group, or join across staged tables without re-fetching from Eurostat. Call eurostat_dataframe_describe first for the table and column names, which differ between the two stagers. Only a single SELECT statement runs: statement chaining, non-SELECT verbs, and functions that read files or external data are rejected. Columns are flat — every dimension is a code column named after the dimension, the measure is obs_value, and the status flag is obs_flag / obs_flag_label; a "_label" companion per dimension exists only on tables eurostat_query_dataset staged, and conf_status / conf_status_label only on tables eurostat_download_dataset staged. Join the two stagers\' tables on dimension codes and time. Do not compare obs_flag across them: a confidential cell reads "|C" in obs_flag on a query table but null in obs_flag with "C" in conf_status on a bulk table.',
